...
function App() {
  ...
  const triggerReload = async () => {
    try {
      ...
      console.log('Server reload status:', data.status);
    } catch (error) {
      console.error('Error triggering server reload:', error);
    }
  };
  ...
  return (
    <div className="flex h-screen" style={{ background: 'linear-gradient(180deg, #0d1b2a, #000)' }}>
      ...
      <div className="w-64 bg-gray-900 text-white flex flex-col p-4 space-y-4 hidden md:flex">
        ...
        <div>
          <button className="bg-gray-700 text-white py-2 px-4 rounded-lg w-full hover:bg-gray-600" onClick = {clearChat}>Clear Chats</button>
        </div>
      </div>
      ...
      <div className="flex flex-col flex-grow h-screen relative">
        ...
        <div className="absolute inset-x-0 bottom-6 mx-auto w-11/12 md:w-1/2 lg:w-1/3">
          <div className="bg-gray-800 rounded-full p-4 flex items-center shadow-lg">
            ...
            <button className="text-white bg-red-800 p-2 rounded-full hover:bg-blue-400" onClick={triggerReload}>
              ⟳
            </button>
          </div>
        </div>
